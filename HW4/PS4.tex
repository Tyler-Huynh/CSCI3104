...
\newpage
    ...
    \begin{proof}[Answer]
    Our final flow-augmenting path would be from  $s \to u \to w \to x \to t$, with a maximum capacity of 1. This path would only have a maximum capacity of 1 because from the edge $u \to w$ we have already pushed a flow of 1 through it thus, it can only hold a maximum flow of 1. 
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{center}
	...
	\end{tikzpicture}
\end{center} 
We will make cuts on the flow network on the edges of $\{u, w\}$, $\{u, v\}$, and $\{s, v\}$. We are making our minimum cuts here because from the vertex of $s$ we can only traverse to the vertex of $u$, such that we can push positive flow on the edge from $\{s, u\}$. Below we have the image with the corresponding minimum cuts of the flow network. \\

\includegraphics[scale=0.5]{Huynh_CSCI3104_HW4Pic.pdf} \\
The value of our flow after making these cuts, would be 4 as we are pushing a total of 4 flow through the edge of $\{s, u\}$. The total capacity of our cut is 7. Thus, we can push through a total of 4 flow as $4 < 7$.
\end{proof}
