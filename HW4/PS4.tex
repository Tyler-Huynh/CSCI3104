...
\begin{center}
	...
	\end{tikzpicture}
\end{center} 
We will make cuts on the flow network on the edges of $c\{u, w\}$, $c\{u, v\}$, and $c\{s, v\}$ as these are the edges that are directly adjacent to our source vertex of $s$ where the capacity of the edge is full,  making the total value of our minimum cut is 7 such that 2 + 2 + 3 = 7. We are making our minimum cuts here because from the vertex of $s$ we can only traverse to the vertex of $u$, such that we can push positive flow on the edge from $\{s, u\}$. Below we have the image with the corresponding minimum cuts of the flow network. \\
