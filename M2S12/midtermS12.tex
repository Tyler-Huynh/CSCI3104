...
Quiz Code (enter in Canvas to get access to the LaTeX template) \dotfill \textbf{zxbhRMJkCG}
...
\section{Standard \standard: Asymptotics I}

\setcounter{required}{\standard-1}
\begin{required} 
Let $f(n) =  (\ln n)^2$ and $g(n) =5n^3$. Determine the relationship that best applies: $f(n) \in O(g(n)), f(n) \in \Theta(g(n))$, or $f(n) \in \Omega(g(n))$. Prove your answer.
Note that you are expected to spell out \textbf{all Calculus details} at the level of Calculus I--II.

    \begin{proof}[Answer]
  \includegraphics[width=0.9\textwidth]{M2S12.pdf}
\end{proof}
